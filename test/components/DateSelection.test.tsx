...
describe("date and selection primitives", () => {
  it("displays a selected date, clears it, and exposes an accessible trigger", async () => {
    ...
    render(
      <DatePicker label="Hire date" onChange={onChange} value="2026-09-17" />,
    );

    expect(
      screen.getByRole("button", { name: /hire date: 17 Sep 2026/i }),
    ).toBeDefined();
  ...
  it("selects only dates inside its configured minimum and maximum", async () => {
    ...
    await user.click(
      screen.getByRole("button", { name: /review date: select a date/i }),
    );
    ...
    await user.click(
      screen.getByRole("button", { name: /september 17.*2026/i }),
    );
  ...
  it("supports partial and complete date ranges and clear behavior", async () => {
    ...
    expect(
      normalizeDateRange({ endDate: "2026-09-17", startDate: "2026-09-30" }),
    ).toEqual({ endDate: "2026-09-30", startDate: "2026-09-17" });
  });
  ...
  it("selects ratings through native radio controls and distinguishes disabled/read-only", async () => {
    ...
    const { rerender } = render(
      <Rate aria-label="Quality" onChange={onChange} />,
    );

    await user.click(screen.getByRole("radio", { name: "4 of 5 stars" }));
    expect(onChange).toHaveBeenCalledWith(4);
    expect(document.activeElement).toBe(
      screen.getByRole("radio", { name: "4 of 5 stars" }),
    );

    rerender(<Rate aria-label="Quality" disabled value={3} />);
    expect(
      (screen.getByRole("radio", { name: "3 of 5 stars" }) as HTMLInputElement)
        .disabled,
    ).toBe(true);
